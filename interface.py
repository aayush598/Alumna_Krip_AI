        ...
        def respond(message, chat_history):
            ...
            try:
                with httpx.Client(timeout=15.0) as client:
                    res = client.post(API_CHAT_URL, json=payload)
                    data = res.json()
        ...
        def new_session():
            try:
                with httpx.Client(timeout=5.0) as client:
                    client.post(API_RESET_URL)
                return [], gr.update(visible=False), gr.update(value="🤖 **Status:** Ready to chat! I'll dynamically extract information as we talk.")
            except Exception as e:
                return [], gr.update(visible=False), gr.update(value=f"❌ Status: Reset failed - {str(e)}")

        def download_profile():
            try:
                with httpx.Client(timeout=10.0) as client:
                    response = client.get(API_DOWNLOAD_URL)
                    if response.status_code == 200:
                        file_path = "student_profile.json"
